...
ERL_OPTS=
# FIXME
DIALYZER=/usr/local/bin/dialyzer
...
ERLANG_XMERL_DIR=/usr/local/lib/erlang/lib/xmerl-1.3.6/include
...
install: tsung doc install_recorder install_controller $(CONFFILE)
	...
	install -m 0644 $(APPFILES) $(DESTDIR)$(TARGETDIR)/ebin/

	install -m 0644 $(SRC) $(DESTDIR)$(TARGETDIR)/src/
...
install_recorder:
	...
	install -m 0644 $(RECORDER_APPFILES) $(DESTDIR)$(RECORDER_TARGETDIR)/ebin

	install -m 0644 $(RECORDER_SRC) $(DESTDIR)$(RECORDER_TARGETDIR)/src
